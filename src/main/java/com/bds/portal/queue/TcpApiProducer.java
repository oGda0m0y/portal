...
public class TcpApiProducer {

	private Producer producer;

	private TcpApiProducer() {
	}

	private static volatile TcpApiProducer instance = null;

	public static TcpApiProducer getInstance() {
		if (instance == null) {
			synchronized (TcpApiProducer.class) {
				if (instance == null) {
					instance = new TcpApiProducer();
					instance.doStart();
				}
			}
		}
	...
	private void doStart() {
		...
		properties.put(PropertyKeyConst.ProducerId, "PID_BDS_API");
	...
	public SendResult send(String body) {
		try {
			Message msg = new Message("bds-api", "task", body.getBytes());
